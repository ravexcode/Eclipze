...
function headers(payload: { name: string; value: string }[] = [], name: string) { return payload.find(item => item.name.toLowerCase() === name.toLowerCase())?.value ?? ""; }
function decode(data = "") { return Buffer.from(data.replace(/-/g, "+").replace(/_/g, "/"), "base64").toString("utf8"); }
function bodyFromPart(part: any): string { if (part.mimeType === "text/plain" && part.body?.data) return decode(part.body.data); return (part.parts ?? []).map(bodyFromPart).find(Boolean) ?? (part.body?.data ? decode(part.body.data) : ""); }
...
export async function listMessages(userId: string, pageToken?: string) {
  ...
  const raw = await Promise.all(ids.map(id => gmailFetch<any>(userId, `/messages/${id}?format=metadata&metadataHeaders=Subject&metadataHeaders=From&metadataHeaders=Date&labelIds=IMPORTANT`)));
