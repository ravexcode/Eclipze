...
export default function RequestsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [requestResponse, notificationResponse, sessionUser] = await Promise.all([
        apiFetch("/api/requests"),
        apiFetch("/api/notifications"),
        getSessionUser(),
      ]);
      if (!requestResponse.ok) throw new Error("Unable to load requests.");
      const data = await requestResponse.json() as { requests: RequestItem[] };
      setRequests(data.requests);
      if (notificationResponse.ok) setUnread((await notificationResponse.json() as { unread: number }).unread);
      if (sessionUser) setRole(sessionUser.role);
    } catch (loadError) {
  ...
  const createRequest = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setError(null);
    const response = await apiFetch("/api/requests", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(form) });
    const data = await response.json().catch(() => null) as { message?: string } | null;
    if (!response.ok) { setError(data?.message ?? "Unable to create request."); return; }
  ...
  return (
    <DashLayout current="requests" router={router}>
      <main className="w-full min-w-0">
        <Heading label="Requests" />
        <div className="mx-auto flex w-full max-w-350 flex-col gap-5 p-4 md:p-6">
          <section className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-xl font-medium">{role === "DEVELOPER" ? "All user requests" : "Your requests"}</p>
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={() => void load()} className="rounded-sm bg-background-card p-2 text-foreground-off hover:bg-background-focus" aria-label="Refresh requests"><IconRefresh size={17} /></button>
              ...
              {role === "USER" && <button type="button" onClick={() => setShowForm(value => !value)} className="flex items-center gap-2 rounded-sm bg-accent px-3 py-2 text-sm hover:brightness-125"><IconPlus size={16} /> New request</button>}
            </div>
          </section>

          {showForm && <form onSubmit={createRequest} className="flex flex-col gap-4 rounded-sm bg-background-card p-5">
            ...
            <div className="flex justify-end gap-2"><button type="button" onClick={() => setShowForm(false)} className="rounded-sm px-3 py-2 text-sm text-foreground-off hover:bg-background-focus">Cancel</button><button type="submit" className="rounded-sm bg-accent px-3 py-2 text-sm hover:brightness-125">Submit request</button></div>
          </form>}

          <section className="flex flex-wrap gap-2 rounded-sm bg-background-card p-3">
            <label className="flex min-w-56 flex-1 items-center gap-2 rounded-sm bg-background-focus px-3 py-2 text-sm text-foreground-off"><IconSearch size={16} /><input value={query} onChange={event => setQuery(event.target.value)} className="w-full bg-transparent text-foreground outline-hidden" placeholder="Search requests" /></label>
