...
export default function RequestDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [response, notificationResponse] = await Promise.all([
      apiFetch(`/api/requests/${params.id}`),
      apiFetch("/api/notifications"),
    ]);
    const data = await response.json().catch(() => null) as { request?: Detail; message?: string } | null;
    if (!response.ok || !data?.request) setError(data?.message ?? "Unable to load request."); else setRequest(data.request);
  ...
  const update = async (field: "status" | "priority", value: string) => {
    const response = await apiFetch(`/api/requests/${params.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ [field]: value }) });
    if (!response.ok) { const data = await response.json().catch(() => null) as { message?: string } | null; setError(data?.message ?? "Unable to update request."); return; }
    await load();
  };
  ...
  const sendMessage = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!body.trim()) return;
    const response = await apiFetch(`/api/requests/${params.id}/messages`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ body }) });
  ...
  return <DashLayout current="requests" router={router}><main className="w-full min-w-0"><Heading label="Requests" /><div className="mx-auto flex w-full max-w-250 flex-col gap-5 p-4 md:p-8">{loading ? <p className="py-8 text-sm text-foreground-off">Loading request…</p> : !request ? <p role="alert" className="text-priority-high">{error ?? "Request not found."}</p> : <>
    <button type="button" onClick={() => router.push("/requests")} className="w-max text-sm text-foreground-off hover:text-foreground">← Back to requests</button>
