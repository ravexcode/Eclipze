...
export async function POST(request: Request, context: Context) {
  ...
  const message = await prisma.$transaction(async transaction => {
    ...
    const recipientIds = user.role === "DEVELOPER" ? [issue.userId] : (await transaction.user.findMany({ where: { role: "DEVELOPER", id: { not: user.id } }, select: { id: true } })).map(developer => developer.id);
    if (recipientIds.length) await transaction.notification.createMany({ data: recipientIds.map(userId => ({ userId, issueId, type: "ISSUE_MESSAGE" as const })) });
    return created;
  });
