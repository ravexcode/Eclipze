...
async function findIssue(userId: string, role: string, id: string) {
  return prisma.issue.findFirst({
    where: { id, ...(role === "DEVELOPER" ? {} : { userId }) },
    include: {
      user: { select: { id: true, username: true, email: true } },
      messages: { orderBy: { createdAt: "asc" }, include: { author: { select: { id: true, username: true, email: true, role: true } } } },
    },
  });
}
...
export async function GET(_request: Request, context: Context) {
  ...
  return NextResponse.json({
    request: {
      ...
      messages: issue.messages.map(message => ({ ...message, createdAt: message.createdAt.toISOString(), updatedAt: message.updatedAt.toISOString() })),
    },
...
export async function PATCH(request: Request, context: Context) {
  ...
  if (body.type !== undefined) {
    const type = enumString(body.type, TYPES, "type");
  ...
  if (user.role === "DEVELOPER") {
    if (body.priority !== undefined) {
      const priority = enumString(body.priority, PRIORITIES, "priority");
    ...
    if (body.status !== undefined) {
      ...
      data.closedAt = status.value === "CLOSED" ? new Date() : null;
    }
  } else if (body.priority !== undefined || body.status !== undefined) {
    return NextResponse.json({ message: "Only Developers can change request priority or status." }, { status: 403 });
  }

  if (!Object.keys(data).length) return badRequest("No request changes provided.");
  ...
  return NextResponse.json({ request: serializeIssue(updated) });
}
